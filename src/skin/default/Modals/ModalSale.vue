<template>
  <b-modal id="bv-modal-sale" hide-footer @show="show">
    ...
    <b-form @submit="onSubmit">
      ...
      <div class="mb-3"  v-if="(formData.interface == 'erc721' || formData.interface == 'erc1155') && (this.$env.VUE_APP_HAS_MULTI_NET != 'on' || (this.$env.VUE_APP_HAS_MULTI_NET == 'on' && this.selectedNet != null))">
        ...
        <b-dropdown menu-class="not-top" variant="token-select">
          ...
          <b-dropdown-item v-for="item in Tokens" @click="chamgePaymentToken(item)"><img class="mr-2" width="30px" height="30px" :src="item.img_url"/>{{ item.symbol }}</b-dropdown-item>
        </b-dropdown>
      </div>
