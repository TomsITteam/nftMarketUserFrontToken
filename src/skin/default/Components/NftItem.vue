<template>
  <div class="last_img_box w-100 bg-white"  @mouseover="mouseover" @mouseleave="mouseleave">
    <router-link :to="'/nft/' + nft.id">
      <img class="lazy nft__item_preview media" v-if="viewImage" :src="nftImageURL" :alt="nft.name" style="border-radius: 10px;">
      <video v-bind:class="{ 'd-none':(viewImage) }" v-if="nft.video_url && isMobileWeb == false" :src="nft.video_url" webkit-playsinline playsinline autoplay loop muted="muted"/>
    ...
    <div class="last_text">
      ...
      <p  v-else-if="parseFloat(nftData.price) <= 0" >{{ $t('nft.not_sale') }}</p>
      <p v-else>
        <strong>
          {{ (nftAmount) }}
