...
export default {
  ...
  deletedNFT : async function(req){
    let signData = await this.getSignData(req.data)
    if(signData == false || signData == undefined){
      return false
    }
  ...
  getTransfers : async function (req) {
    let res = await this.request('GET', '/nft/' + req.data.nft_id + '/transfers', req.data);
    return res
  },

  getOwners : async function (req) {
    let res = await this.request("GET", '/nft/' + req.data.nft_id + '/owners', req.data);
    return res
  },

  getBids : async function (req) {
    let res = await this.request("GET", "/nft/" + req.data.nft_id + '/bids', req.data);
    return res
  }
};
