  ...
  watch:{
    sellerData(val){
      ...
      if(this.$env.VUE_APP_HAS_MULTI_NET == "on"){
        contractAddress = this.$env['VUE_APP_' + this.nftData.net + '_MULTINFT_CONTRACT_ADDRESS']
      }else{
        contractAddress = this.$env.VUE_APP_MULTINFT_CONTRACT_ADDRESS
      }

      if(this.nftData.token.tokenAddress != '0x0000000000000000000000000000000000000000'){
        this.updateAllow(val.token.tokenAddress,contractAddress)
