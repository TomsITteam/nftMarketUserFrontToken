...
export default {
  ...
  computed:{
    ...
    nftBuyerButtonType(){
      ...
      if(this.$wallet.getWallet().isWallet() == false){
        ...
        return "bid"
      }else if(this.nftData.offer != undefined && this.nftData.offer.isForSale == true && this.auctionRemainingTime == "00:00:00"){
        return "nonOwnerAuctionEnd"
      }
    ...
    nftOwnerButtonType(){
      ...
      if(this.unconfirmed == true){
        ...
        return "auctionEnd"
      }else if(this.nftData.offer != undefined && this.nftData.offer.isForSale == true && this.nftData.bid != undefined && this.nftData.bid.hasBid == false && this.auctionRemainingTime == "00:00:00"){
        return "cancelAuction2"
  ...
  methods: {
    ...
    async getNft(nft_id) {
      ...
      if(this.$env.VUE_APP_HAS_MULTI_NET == "on"){
        contractAddress = this.$env['VUE_APP_' + this.nftData.net + '_SINGLENFT_CONTRACT_ADDRESS']
      }else{
        contractAddress = this.$env.VUE_APP_SINGLENFT_CONTRACT_ADDRESS
      }

      if(this.nftData.interface == 'erc721' && this.nftData.token.tokenAddress != '0x0000000000000000000000000000000000000000') {
        this.updateAllow(this.nftData.token.tokenAddress, contractAddress)
    ...
    async getTransfers() {
      ...
      let result = await this.$api.getTransfers({
        data : {
          nft_id : this.$route.params.nft_id,
          offset : this.offset
        }
      })
      ...
      this.transfersTotal = result.total
    },
    async getOwners() {
      let result = await this.$api.getOwners({
        data : {
          nft_id : this.$route.params.nft_id,
          offset : this.offset
        }
      ...
      this.ownersTotal = result.total
    },
    async getBids() {
      let result = await this.$api.getBids({
        data : {
          nft_id : this.$route.params.nft_id,
          offset : this.offset
        }
      ...
      this.bidsTotal = result.total
    },
    downScroll() {
      let scroll = Math.floor((
        window.scrollY /
        (document.body.clientHeight -
          window.innerHeight)
      ) * 100);

      if((scroll > 10 && scroll <= 15) && this.transLoading && (this.transfers.length < this.transfersTotal || this.owners.length < this.ownersTotal)) {
  ...
  mounted() {
    document.addEventListener('scroll', this.downScroll)
  },
  unmounted() {
    document.removeEventListener('scroll', this.downScroll)
  },
}
